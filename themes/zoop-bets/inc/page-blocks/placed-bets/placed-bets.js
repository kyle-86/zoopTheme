window.addEventListener('load', function () {
  ...
  jQuery(document).ready(function ($) {
    ...
    $.getJSON('/zoop/wp-json/zoop/v1/bets', function (userBets) {
      ...
      let filteredData = userBets.filter(element => {
        ...
        return userIdAsInt === currentUserId && eventTime.isAfter(currentDate);
      });
      console.log(filteredData);
      DataTable.datetime('MMM Do YYYY [at] hh:mm A');
      table = $('#placedBets').on('init.dt', function () {
        ...
        columns: [
          ...
          },
          {
            ...
            render: function (data, type, row) {
              if (type === 'display') {
                return '<div class="editBet"> <i class="fa-solid fa-pencil"></i> </div>';
              }
              return data;
            }
          },
          {
            ...
            render: function (data, type, row) {
              if (type === 'display') {
                return '<div class="trashBet"> <i class="fa-solid fa-trash-can"></i> </div>';
              }
              return data;
            }
          }
  ...
  function customSearchFunction() {
    ...
    let firstInputField = jQuery('#placedBets thead input');
  ...
  jQuery('#placedBets tbody').on('click', '.editBet', function () {
    // Get the clicked DataTable cell
    let cell = jQuery(this);
    // Get the DataTable row associated with the clicked cell
    let row = cell.closest('tr');
